...
Proof.
  intros.
  Check _ : Filter (prod _ _) (M A).
  Check _ : ElemOf K D.
  Check _ : forall x : prod K A, Decision ((fun pat : prod K A => match pat return Prop with | @pair _ _ k _ => @elem_of K D H7 k X1 end) x).
  Check _ : forall x : prod K A, Decision ((fun pat : prod K A => match pat return Prop with | @pair _ _ k _ => not (@elem_of K D H7 k X1) end) x).
